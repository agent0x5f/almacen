	...
	{		
		if(tabla[x][2]=="F")//BUG: debo revisar cuantos operadores usa la funcion,x ahora solo funciona de 1
		{
			...
			*/
			...
			//corrigiendo error de -1,pregunto si es ya un numero,ya que antes se suponia que eran siempre letras
			//si es una letra..
			if((tabla[x][1]>="a" && tabla[x][1]<="z") || (tabla[x][1]>="A" && tabla[x][1]<="Z"))
			{
			cout<<obten_direccion(tabla[x][1])<<endl;			
			salida<<obten_direccion(tabla[x][1])<<endl;
			}
...
void menu()
{
	...
	do{
		...
		cout<<"15.-procesa_reservadas"<<endl;
		cout<<"16.-calcula_direcciones"<<endl;
		...
		case 15:	procesa_reservadas(); break;
		case 16:	calcula_direcciones(); break;
		default: break;
		}
